...
fn mint_and_send_to_parent<T: Config>(owner: T::AccountId, collection_id: CollectionIdOf<T>, n: u32) {
	for i in 1..n {
...
) -> (T::AccountId, T::AccountId, CollectionIdOf<T>, ItemIdOf<T>, ResourceId) {
...
mod benchmarks {
	...
	fn create_collection<T: Config>() {
		...
		let collection_id: CollectionIdOf<T> =
	...
	fn mint_nft<T: Config>() {
		...
		assert_last_event::<T>(
			Event::NftMinted { owner: owner_enum, collection_id, nft_id }.into(),
		);
	}
	...
	fn mint_nft_directly_to_nft<T: Config>(n: Linear<1, { T::NestingBudget::get() - 1 }>) {
		...
		let nft_child_id: ItemIdOf<T> = T::Helper::item(n);
		let owner_tuple =
		...
		assert_last_event::<T>(
			Event::NftMinted { owner: owner_tuple, collection_id, nft_id: nft_child_id }.into(),
		);
	}

	#[benchmark]
	fn destroy_collection<T: Config>() {
		let owner: T::AccountId = whitelisted_caller();
		let collection_index = 1;
		let collection_id = create_test_collection::<T>(owner.clone(), collection_index);
	...
	fn send_to_nft<T: Config>(n: Linear<1, { T::NestingBudget::get() }>) {
		...
		let parent_nft =
			AccountIdOrCollectionNftTuple::CollectionAndNftTuple(collection_id, nft_id1);
	...
	fn reject_nft<T: Config>(n: Linear<1, { T::NestingBudget::get() }>) {
		...
		assert_last_event::<T>(
			Event::NFTRejected { sender: bob, collection_id, nft_id: child_nft }.into(),
		);
	}

	#[benchmark]
	fn change_collection_issuer<T: Config>() {
		let alice: T::AccountId = whitelisted_caller();
		let collection_index = 1;
		let collection_id = create_test_collection::<T>(alice.clone(), collection_index);
		...
		assert_last_event::<T>(
			Event::IssuerChanged { old_issuer: alice, new_issuer: target, collection_id }.into(),
		);
	}

	#[benchmark]
	fn set_property<T: Config>() {
		let alice: T::AccountId = whitelisted_caller();
		...
		assert_last_event::<T>(
			Event::PropertySet { collection_id, maybe_nft_id: Some(maybe_nft_id), key, value }
				.into(),
		);
	}

	#[benchmark]
	fn lock_collection<T: Config>() {
		let alice: T::AccountId = whitelisted_caller();
		let collection_index = 1;
		let collection_id = create_test_collection::<T>(alice.clone(), collection_index);
	...
	fn replace_resource<T: Config>() {
		...
		let _ = RmrkCore::<T>::add_basic_resource(
		...
		let resource = ResourceTypes::Basic(BasicResource {
			metadata: stbd::<T>("replaced basic test metadata"),
		}); // new_resource
		...
		assert_last_event::<T>(
			Event::ResourceReplaced { nft_id, resource_id, collection_id }.into(),
		);
	}
	...
	fn accept_resource<T: Config>(n: Linear<1, { T::NestingBudget::get() }>) {
		...
		assert_last_event::<T>(
			Event::ResourceAccepted { nft_id, resource_id, collection_id }.into(),
		);
	}
	...
	fn remove_resource<T: Config>(n: Linear<1, { T::NestingBudget::get() }>) {
		...
		// Bob accepts new resource
		let _ = RmrkCore::<T>::accept_resource(
		...
		assert_last_event::<T>(
			Event::ResourceRemoval { nft_id, resource_id, collection_id }.into(),
		);
	}
	...
	fn accept_resource_removal<T: Config>(n: Linear<1, { T::NestingBudget::get() }>) {
		...
		// Only collection issuer can request resource removal
		let _ = RmrkCore::<T>::remove_resource(
		...
		assert_last_event::<T>(
			Event::ResourceRemovalAccepted { nft_id, resource_id, collection_id }.into(),
		);
	}
